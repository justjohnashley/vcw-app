...
import com.pural_ba3a.vulcanwash.databinding.FragTwoadminBinding;

import java.util.ArrayList;
import java.util.List;


public class FragTwoAdmin extends Fragment {

    FragTwoadminBinding binding;
    private FirebaseAuth mAuth;
    private FirebaseUser user;
    private FirebaseFirestore firestore;

    private OrderAdapterAdmin activeAdapter, historyAdapter, archivedAdapter;
    private List<OrderAdmin> activeOrders = new ArrayList<>();
    private List<OrderAdmin> historyOrders = new ArrayList<>();
    private List<OrderAdmin> archivedOrders = new ArrayList<>();


    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragTwoadminBinding.inflate(getLayoutInflater(), container, false);
    ...
    private void setupRecyclerViews() {
        ...
        activeAdapter = new OrderAdapterAdmin(activeOrders);
        historyAdapter = new OrderAdapterAdmin(historyOrders);
        archivedAdapter = new OrderAdapterAdmin(archivedOrders);
    ...
    private void fetchOrders() {
        ...
        firestore.collection("orders")
                .whereEqualTo("shopId", shopId) // Retrieve only orders for the logged-in user
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    ...
                    for (DocumentSnapshot document : queryDocumentSnapshots) {
                        OrderAdmin order = document.toObject(OrderAdmin.class);
